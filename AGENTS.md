...
- **模式扩展页采集**：竞技场入口实际为“使用门票进入”页（未点击继续），战斗代码进入卡组后为代码输入页（未输入/开始）；两页分别被误判为 `MODE_MENU` 和 `UNKNOWN`。战役模式实际可进入战役列表，再进入“德国战役”详情页；两页均被误判为 `MODE_MENU`/`UNKNOWN`。赛事模式实际为赛事列表，含“排行榜”和需要报名费的赛事卡，未点击报名；该页被误判为 `CARD_COLLECTION`。
- **排行榜与账户侧页采集**：赛事排行榜可打开并按页浏览，实际排行榜页返回 `UNKNOWN`；主页头像打开“更改头像”页，好友图标打开好友/邀请/近期对手面板，两者分别返回 `UNKNOWN` 和 `MODE_MENU`。主页设置中的“百科”“E.U.L.L.A.”及“兑换代码”弹窗均可安全打开，暂未接入页面识别；兑换码未输入。
- **每周军需箱页未命中**：主页“军需箱”可打开“每周军需箱”进度页，显示基础/普通/限定/特殊/精英五档箱子和剩余时间，未点击箱子领取；实时探针返回 `UNKNOWN`。关闭后设备已回到主页。
