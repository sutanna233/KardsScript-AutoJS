...
## 2026-08-26 悬浮窗尺寸调整
- 用户反馈悬浮窗过大；已将目标尺寸调整为 `180x56`，位置调整为 `(1080,80)`，保留“暂停”和“停止”两个固定点击区域。
- 修改后必须重新打包 APK，并检查窗口不超出 1280x720：右边界为 1260。
...
## 2026-08-23 大版本可靠性更新
- 回合超时专项第一阶段已记录在 `docs/reliability-update-v2.md`：缩短出牌/单位动作/导航确认等待，减少 HQ 扫描步数和 OCR 瞬态重试等待，出牌重试上限降为 2。
- 只允许合规稳定性改进（固定安全节奏、有限重试、异常退避、安全停机）；不实现随机人类化、反作弊探测、检测规避或 AFK 绕过。
- 参数下调尚未替代训练模式有界实机验证；低性能设备和网络抖动可能需要回滚到上一组参数。

- README 顶部必须醒目标注：使用自动化脚本有违反游戏规则、账号限制或封禁风险，用户应谨慎评估并自行承担后果；项目作者及贡献者不对封号、限制或其他损失负责。

## 2026-08-23 开发状态声明
- README 顶部和已知限制中必须说明项目仍在开发，存在未知 Bug，脚本可能卡住、停止运行或需要手动恢复，不保证稳定挂机。
