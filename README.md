...
1. **分辨率锁定 1280×720**：所有坐标和模板基于此分辨率，其他分辨率会误操作
2. **费用 OCR 尚未稳定**：出牌依赖费用徽章颜色（橙色=可出）而非精确数字
3. **7~9 张手牌的动态布局**：扇形手牌在 6 张以内已校准，7~9 张仍需进一步验证
4. **卡牌内容识别有限**：可以识别费用和卡牌类型（步兵/坦克/空军等），但无法读取卡牌名称和详细效果
5. **网络依赖**：KARDS 是在线游戏，断线可能导致对局失败
6. **debug 签名**：APK 为 debug 构建，部分安全软件可能标记为未知来源
